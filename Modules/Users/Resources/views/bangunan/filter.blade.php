<script>
    ...
    function changeSubject(val) {
      ...
      if (subject_value == 'id_province') {
            ...
            $.each(<?= json_encode($province) ?>, function(index, value) {
                            operator_value.options[operator_value.options.length] = new Option(value.province_name,value.id_province);
                        });
            ...
            for (i = operator_value.options.length - 1; i >= 0; i--)
                operator_value.remove(i);
            $.each(<?= json_encode($city) ?>, function(index, value) {
                            operator_value.options[operator_value.options.length] = new Option(value.city_name,value.id_city);
                        });
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="col-md-4">
                                    <select name="subject" class="form-control input-sm select2" placeholder="Search Subject" onChange="changeSubject(this.name)" style="width:100%">
                                        ...
                                        <option value="name" @if ($con['subject'] == 'name') selected @endif>Name Customer</option>
                                        <option value="nik" @if ($con['subject'] == 'nik') selected @endif>NIK</option>
                                        <option value="id_city" @if ($con['subject'] == 'id_city') selected @endif>Kota</option>
                                        <option value="id_province" @if ($con['subject'] == 'id_province') selected @endif>Provinsi</option>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <select name="operator" class="form-control input-sm select2" placeholder="Search Operator" id="test" style="width:100%">
                                        @if($con['subject'] == 'id_province')
                                            @foreach($province as $pt)
                                                <option value="{{$pt['id_province']}}" @if ($pt['id_province']  == $con['operator']) selected @endif>{{$pt['province_name']}}</option>
                                            @endforeach
                                        @elseif($con['subject'] == 'id_city')
                                            @foreach($city as $ct)
                                                <option value="{{$ct['id_city']}}" @if ($ct['id_city']  == $con['operator']) selected @endif>{{$ct['city_name']}}</option>
                                            @endforeach
                                        @else
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="col-md-4">
                                    <select name="subject" class="form-control input-sm select2" placeholder="Search Subject" onChange="changeSubject(this.name)" style="width:100%">
                                        ...
                                        <option value="id_city">Kota</option>
                                        <option value="id_province">Provinsi</option>
                                    </select>
                                </div>
                                <div class="col-md-4">
                                    <select name="operator" class="form-control input-sm select2" placeholder="Search Operator" id="test" style="width:100%">
