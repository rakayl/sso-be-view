...
@section('content')
    ...
    <a href="{{url('plastic-type')}}" class="btn green" style="margin-bottom: 2%;"><i class="fa fa-arrow-left"></i> Back</a>
    ...
    <div class="portlet light bordered">
        ...
        <div class="portlet-body form">
            <form class="form-horizontal" role="form" action="{{url('plastic-type/update')}}/{{$detail['id_plastic_type']}}" method="post">
                <div class="form-body">
                    <div class="form-group">
                        ...
                        <div class="col-md-4">
                            <input type="text" class="form-control" name="plastic_type_name" value="{{$detail['plastic_type_name']}}" placeholder="Plastic Type Name" required>
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-md-4">
                            <div class="input-icon right">
                                <select  class="form-control select2 select2-multiple-product" multiple name="group_filter[]" data-placeholder="Select" required>
                                    <option></option>
                                    <?php
                                        $arr = array_column($detail['outlet_group'], 'id_outlet_group');
                                    ?>
                                    @foreach($outlet_group_filter as $val)
                                        <option value="{{$val['id_outlet_group']}}" @if(in_array($val['id_outlet_group'],$arr)) selected @endif>{{$val['outlet_group_name']}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
