        ...
        $(document).ready(function(){
            $('#sortable').sortable({
                update: function (event, ui) {
                    ...
                    $.post("{{url('setting/faq/kontraktor/sort/update')}}", $('#formFaqSort').serialize(), function(result){
                        if(result['status'] == 0 ){
                            location.reload();
                        }
                    });
                        ...
                        @if($index != $total)
                            @if(strlen($result[$index]['question']) > 85)
                                <div class="col-md-3" style="cursor: all-scroll;">
                                    <input type="hidden" name="id_faq_kontraktor[]" value="{{ $result[$index]['id_faq_kontraktor'] }}">
