...
@section('content')
    ...
    <div class="portlet light bordered">
        <div class="portlet-title">
            <div class="caption">
                <span class="caption-subject font-dark sbold uppercase font-blue">New Payment Method</span>
            </div>
        </div>
        <div class="portlet-body form">
            <form class="form-horizontal" role="form" action="{{ url('payment-method/store') }}" method="post" enctype="multipart/form-data">
                <div class="form-body">
                    <div class="form-group">
                        ...
                        <div class="col-md-7">
                            <div class="input-icon right">
                                <input type="text" placeholder="Payment Method Name" class="form-control" name="payment_method_name" value="{{ old('payment_method_name') }}" required>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="input-icon right">
                                <select class="form-control" name="id_payment_method_category" required>
                                    ...
                                    @foreach ($payment_method_category as $item)
                                        <option value="{{$item['id_payment_method_category']}}" @if($item['id_payment_method_category'] == old('id_payment_method_category')) selected @endif>{{ $item['payment_method_category_name'] }}</option>
                                    @endforeach 
                                </select>
                            </div>
                        </div>
