...
@section('content')
    ...
    <form role="form" class="form-horizontal" action="{{url()->current()}}?filter=1" method="POST">
        {{ csrf_field() }}
        @include('merchant::artikel.filter_list')
    </form>
        ...
        <table class="table table-striped table-bordered table-hover">
            ...
            @if(!empty($data))
                @foreach($data as $val)
                    <tr>
                        <td>
                            <a class="btn btn-sm btn-info" href="{{ url('artikel/detail', $val['id_artikel']) }}"><i class="fa fa-search"></i></a>
                            <a class="btn btn-sm btn-danger" href="{{ url('artikel/delete',$val['id_artikel']) }}"><i class="fa fa-trash"></i></a>
                        </td>
                        <td>{{ $val['title'] }}</td>
                        <td>{{ $val['category'] }}</td>
