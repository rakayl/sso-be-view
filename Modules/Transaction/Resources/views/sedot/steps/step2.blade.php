	...
	<form class="form-horizontal" id="form_interview" role="form" action="{{url('transaction/detail/step')}}" method="post" enctype="multipart/form-data">
		<div class="form-body">
			...
			<div class="form-group">
				...
				<div class="col-md-6">
                                    <input  @if($detail['transaction_status_code']==4&&$detail['step_number']!=1) disabled @endif type="datetime-local" class="form-control form-filter input-sm datetimepicker" name="time_confirm" placeholder="From"  value="{{date('Y-m-d H:i',strtotime($detail['trasaction_sedot_wc']['time_confirm']??date('Y-m-d H:i')))}}"  min="{{date('Y-m-d H:i')}}" data-date-format="yyyy-mm-dd hh:ii">
                                </div>
			</div>
