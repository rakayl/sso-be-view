...
@section('page-script')
    ...
    <script type="text/javascript">
        ...
        function sortCategory() {
            ...
            $.ajax({
                type : "POST",
                url : "{{ url('news/category/position/assign') }}",
                data : data,
        ...
        function sortProduct(e) {
            ...
            var data = {_token: token, news_ids: ids};

            $.ajax({
                type : "POST",
                url : "{{ url('news/position/assign') }}",
                data : data,
                success : function(result) {
                    if (result.status == "success") {
                        toastr.info("News has been sorted.");
                    }
                    else if(result.messages != ""){
                        toastr.warning(result.messages);
                    }
                    else {
                        toastr.warning("Something went wrong. Failed to sort news.");
                    }
                }
            });
        }
...
@section('content')
    ...
    <div class="portlet light bordered">
        <div class="portlet-title">
            <div class="caption">
                <span class="caption-subject sbold uppercase font-blue">List News By Position</span>
            </div>
        </div>
        <div class="portlet-body">
            <div class="alert alert-info">
                Drag and drop the item to sort news or categories
            </div>
            <div class="row">
                {{-- category --}}
                <div class="col-md-3 col-sm-3 col-xs-3 category-position">
                    <ul id="sortable-category" class="nav nav-tabs tabs-left">
                        @foreach($category as $key => $cat)
                            @if(empty($cat['child']))
                                <li class="main-menu {{ ($key==0 ? 'active' : '') }}" data-id="{{ $cat['id_news_category'] }}">
                            ...
                            <li class="main-menu {{ ($key==0 ? 'active' : '') }}" data-id="{{ $cat['id_news_category'] }}">
                                ...
                                <ul class="nav sortable-category-sub">
                                    <li class="sub-menu-parent ui-state-disabled-sub {{ ($key==0 ? 'active' : '') }}">
                                        <a href="#tab_{{$key}}" data-toggle="tab"> {{$cat['category_name']}} </a>
                                    </li>
                                    @foreach($cat['child'] as $keyC => $child)
                                    <li class="sub-menu ui-state-disabled" data-id="{{ $child['id_news_category'] }}">
                                    ...
                                    <ul class="nav sortable-product">
                                        @if(!empty($news))
                                            @foreach($news as $value)
                                                @if (($value['category'][0]['id_news_category']??false) == $cat['id_news_category'])
                                                <li class="product-li" data-id="{{ $value['id_news'] }}">
                                                    <i class="sort-icon fa fa-arrows"></i>
                                                    {{ $value['news_title'] }}
                                                </li>
                                                @endif
                            ...
                            @else
                                ...
                                <div class="tab-pane" id="tab_{{$key}}_{{$keyC}}">
                                    <ul class="nav sortable-product">
                                        @if(!empty($news))
                                            @foreach($news as $value)
                                                @if (($value['category'][0]['id_news_category']??false) == $child['id_news_category'])
                                                <li class="product-li" data-id="{{ $value['id_news'] }}">
                                                    <i class="sort-icon fa fa-arrows"></i>
                                                    {{ $value['news_title'] }}
                                                </li>
